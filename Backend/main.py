...
import math
# prof_names=['meera', 'parkavi', 'mallegowda', 'srinidhi', 'veena', 'divakar']
# class_names=['cns', 'da', 'hpc', 'dl', 'pmee', 'cc']
# lab_names=['da lab', 'hpc lab']
prof_names = []
class_names = []
lab_names = []

Group.groups = [Group("a", 50), Group("b", 50),Group("c", 50),Group("d", 50),Group("e", 50)]
...
time_slots=[["9:00","9:55"],["9:55","10:50"],["11:05","12:00"],["12:00","12:55"]]
lab_slots=[["9:00","10:50"],["11:05","12:55"]]
slots=[]
for i in range(5):
	if days[i]!="Fri":
		for j in range(4):
...
all_slots=time_slots+lab_slots
cpg = []
lts = []
slots = []
sched_dict={}
...
def convert_dict():
	for i in conv_dict.keys():
		for j in days:
			temp_dict=sched_dict[i][j]
			temp_dict["day"]=j
			conv_dict[i].append(temp_dict)
def bits_needed(x):
...
def print_chromosome(chromosome):
    ...
    day=Slot.slots[int(slot_bits(chromosome), 2)].day
    time=Slot.slots[int(slot_bits(chromosome), 2)].start+'-'+Slot.slots[int(slot_bits(chromosome), 2)].end
    group=Group.groups[int(group_bits(chromosome), 2)].name
    sub=CourseClass.classes[int(course_bits(chromosome), 2)].code
    sched_dict[group][day][time]=sub
...
def main(data): #
    global cpg, prof_names, class_names, lab_names
    for i in data:
        faculty = i['facultyAssigned']
        faculty = faculty.lower()
        theory = int(i['courseTheory'])
        lab = int(i['courseLab'])
        clss = i['courseName']
        if faculty not in prof_names:
            prof_names.append(faculty)
        if theory!=0 and lab==0:
            class_names.append(clss.lower())
        elif theory==0 and lab==1:
            lab_names.append(clss.lower())
    for i in prof_names:
	    Professor.professors.append(Professor(i))
    for i in class_names:
	    CourseClass.classes.append(CourseClass(i))
    for i in lab_names:
	    CourseClass.classes.append(CourseClass(i,is_lab=True))
    for i in data:
        theory = int(i['courseTheory'])
        lab = int(i['courseLab'])
        name = i['courseName']
        ...
        faculty = faculty.lower()
        if theory > 0 and lab == 0:
            for j in range(theory//2):
                cpg.extend([CourseClass.find(name),Professor.find(faculty),Group.find("a")])
            for j in range(theory//2):
                cpg.extend([CourseClass.find(name),Professor.find(faculty),Group.find("b")])
        elif lab == 1:
            print(name, faculty)
            cpg.extend([CourseClass.find(name),Professor.find(faculty),Group.find("a")])
            cpg.extend([CourseClass.find(name),Professor.find(faculty),Group.find("b")])
